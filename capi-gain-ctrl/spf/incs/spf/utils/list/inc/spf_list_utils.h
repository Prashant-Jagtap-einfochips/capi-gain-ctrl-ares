...
/*
  ...
  return: none
*/
void spf_list_buf_pool_init(POSAL_HEAP_ID heap_id, uint32_t num_arrays);
...
/*
  Insert a node before a given node (updates head if needed)
...
/*
 * Deletes the node pointed to by node_ptr, and updates node_ptr with a pointer to the next node
 * If the last node is deleted, updates pointer to NULL
 *
 *   param[in] pool_used     : TRUE if gk list pool was used for allocating memory for list nodes.
                             FALSE if malloc was used.
 */
