...
// clang-format off
/*
$Header: //source/qcom/qct/platform/adsp/proj/addons_audio/spf/incs/spf/interfaces/module/capi/capi_intf_extn_metadata.h#3 $
*/
// clang-format on
...
/** specifies the control flags the tracking metadata. */
   ...
   union
   {
      struct
      {
         ...
         /**< Indicates if the tracking information needs to be allocated only from the specified heap.
              ...
              - #MODULE_CMN_MD_TRACKING_USE_SPECIFIED_HEAP_MANDATORY
               @tablebulletend */

         uint32_t enable_cloning_event : 1;
         /**< Indicates if the clients needs to be notified through an event when the tracking MD
          *   is cloned in the framework.

              @valuesbul
              - #MODULE_CMN_MD_TRACKING_DISABLE_CLONING_EVENT
              - #MODULE_CMN_MD_TRACKING_ENABLE_CLONING_EVENT
               @tablebulletend */

         uint32_t requires_custom_event : 1;
         /**< Indicates if the metadata would need custom tracking event handling

              @valuesbul
              - #MODULE_CMN_MD_TRACKING_USE_GENERIC_EVENT
              - #MODULE_CMN_MD_TRACKING_USE_CUSTOM_EVENT
               @tablebulletend */
...
/** Specifies that client does not need any cloning event */
...
/** Specifies that the tracking event uses generic implementation */
#define MODULE_CMN_MD_TRACKING_USE_GENERIC_EVENT 0

/** Specifies that the tracking event would need custom implementation
 *  example : EOS from HLOS client used custom implementation and EOS
 *  from OLC uses generic implementation*/
#define MODULE_CMN_MD_TRACKING_USE_CUSTOM_EVENT 1

/** Payload structure specific for the metadata tracking information. */
...
/** The metadata can create a tracking reference based on client configuration.
...
struct module_cmn_md_tracking_t
{
   ...
   /**< Specifies the heap id for allocating tracking payload. */
};
...
struct intf_extn_param_id_metadata_handler_t
{
   /** Version of this structure (currently v1).
       ...
       example : if module is compiled for version 2 and framework uses version 1,
                 module should only use API features from version 1 specified by
   ...
   */
   ...
   /**
     Allocates memory for metadata with tracking, creates a list node, and inserts the new
     list node to the tail of a given list. Also create reference counter for tracking
     ...
     @par
     The metadata would create a tracking reference. The tracking reference
     would have the payload to raise an event when the metadata is either
