...
// clang-format off
/*
$Header: //source/qcom/qct/platform/adsp/proj/addons_audio/spf/incs/spf/interfaces/module/capi/capi_types.h#3 $
*/
// clang-format on
...
/** Flags that are passed with every input buffer and must be filled by the
    ...
    marker_eos for modules. If the framework's method of handling marker_eos/end_of_frame
    does not address a module's requirements, the module must implement #INTF_EXTN_METADATA extension
   ...
   */
   struct
   {
      ...
      /**< Specifies whether the timestamp is valid.
           ...
           For single-input-single-output modules the
           framework propagates time-stamp & related flags
           (time-stamp valid,continue) from input to output
      ...
      /**< Specifies whether the buffer has an end of frame.
           ...
           For raw-compressed-data, usually end_of_frame indicates that the buffer has
           integral number of encoded frames.

           end_of_frame is also set for discontinuities (timestamp discontinuity, EOS).

           If a module does not implement #INTF_EXTN_METADATA extension,
      ...
      /**< Indicates that this data is the last valid data from the upstream
           ...
           Following is how typical flushing EOS propagation works
           ...
           2. EOS undergoes algorithmic or buffering delay (if applicable)
                 marker_eos flag is set on the input.
                 EOS metadata is moves internal to the module. input port metadata list is cleared.
           3. EOS goes to output or gets destroyed or gets converted to non-flushing EOS.
                 marker_eos flag moves to output.
                 input marker_eos is cleared. internal metadata list is cleared.
                 In case EOS moves to output, marker_eos on output is set. output port metadata list is populated.

           Thus, on the input port marker_eos is set whenever a new EOS arrives or
           when previous EOS is stuck inside the module due to delays.
           Output marker_eos is set only when there's a EOS metadata in the output list.

           marker_eos is taken care by the framework for modules that don't implement INTF_EXTN_METADATA.
           For others, EOS metdata propagation including marker_eos handling, is taken care by
           #intf_extn_param_id_metadata_handler_t::metadata_propagate() in #INTF_EXTN_METADATA.
           However, for create/destroy/clone etc, module must take care of setting/clearing marker_eos.
      ...
      /**<  If time-stamp continue flag is set, then the time-stamp field must not be read.
            Time-stamp values based on previously set time-stamp must continue.
            If the previous time-stamp value are invalid, then this flag doesn't apply.
            If ts_continue is reset and ts_valid is set, we need to sync to input time-stamp.
            This field should be used only for raw compressed data formats

           @valuesbul
           - 0 -- TS continue is set as FALSE
           - 1 -- TS continue is set as TRUE @tablebulletend */
 ...
 * Unlike raw compressed, there's no payload following this struct for media format
 * specific payload, only channel mask follows.
 */
struct capi_deinterleaved_raw_compressed_data_format_t
{
   uint32_t minor_version;
   /**< Minor version for this payload. Only version 1 supported now. */
    ...
    *    channel mask is derived by bit-shifting channel map. channel mask = 1 << channel type.
    *    Channel type values are defined in

        @values PCM_CHANNEL_* types defined in
        @xrefcond{Q3,80-NF775-1,80-NA611-1}.
