...
// clang-format off
/*
$Header: //source/qcom/qct/platform/adsp/proj/addons_audio/spf/incs/spf/interfaces/module/capi/capi_events.h#3 $
*/
// clang-format on
...
typedef enum capi_event_id_t {
   ...
   /**< Indicates the KPPS requirement of the module.
        ...
        requirement changes. @vertspace{4}
         ...
         KPPS (Kilo-Packets-Per-Second), where 'packets' is a Hexagon Processor specific
         terminology multiple instructions grouped for parallel execution.
         When CAPI is used for non-Hexagon processors, this event must be re-interpreted as
         Kilo-Instructions-Per-Second or such similar property which indicates processing load per second.
  ...
  is not known:
   ...
   /**< Sends data to the SPF client application processor. @vertspace{4}
        ...
        Payload: #capi_event_data_to_dsp_client_v2_t @vertspace{6} */

   CAPI_EVENT_DYNAMIC_INPLACE_CHANGE = 16,
   /**< Event ID used by the module to inform dynamic change in inplace property.

        Module changing itself to inplace must be,
          1. SISO as long as its inplace, if it's MIMO when the event is raised it will errored out.
          3. Input media format and output media format must be equal.
          3. Port thresholds must be equal.

        Module must change itself to non-inplace,
          1. If it become multi-in/multi-out at point of time.
          2. If input and output media formats are different.
          3. If input and output thresholds are different.

      Payload: #capi_event_dynamic_inplace_change_t @vertspace{6} */

   CAPI_EVENT_HW_ACCL_PROC_DELAY = 17,
   /**< Event ID used by the module to indicate the HW accelerator delay.

      This delay applies to the modules utilizing hardware accelerators for off-loading partial/complete processing. It
      is the maximum time in microseconds taken by the H.W accelerator for the off-loaded processing for the provided
      input frame. H.W accelerator processing happens in a synchronous manner such that the caller waits for the frame
      processing to finish. This delay does not include algorithmic delay due to signal processing in the H.W
      accelerator.
   ...
   /**< Event ID used by the module to indicate if it wants to vote for or against Island.
      ...
      If this event is raised by a module placed in an non-island conatiner, it will be ignored.
      Against vote will be honored immediately. 
...
/** Deprecated. See #capi_event_dynamic_inplace_change_t.
...
struct capi_event_dynamic_inplace_change_t
{
   ...
   /**< '0' = indicates module changed to non-inplace.
        'Non zero' = indicates that module changed to inplace. 
      */
};
...
{
   uint32_t delay_in_us; 
   /**< HW  requirement of the module. Default value is 0*/
};
