...
/*==============================================================================
  @file metadata_api.h
  @brief This file contains metadata IDs. Definitions are not supposed to be here.
  definitions will be in module_cmn_metadata.h
...
struct metadata_header_t
{
    ...
    *
    *   @values{for bit 3}
    *      - 0 -- MD_HEADER_FLAGS_TRACKING_EVENT_POLICY_LAST ( raise event for only the last drop/consumption of md in
    * SPF)
    ...
    *               metadata is applicable from sample at (data buffer +  (sample_offset * bytes_per_sample * num
    * channels ) For deinterleaved packed data, metadata is applicable from sample at (data buffer + offset *
    * bytes_per_sample) in each channel For deinterleaved data unpacked, metadata is applicable from
    * sample at (channel data buffer + (sample_offset * bytes_per_sample)) Effective sample_offset in
    * above cases must be less than buffer end
    */
...
struct metadata_tracking_event_t
{
   ...
   */
    ...
    *
    *  Client Metadata:
    *         For Metadata from the client, this is specified in the medata API header and the same value would be used
    * here. Module metadata to client: If the metadata is generated by a module in SPF, the module would need to specify
    * the token. It can be configured to the module by the client, if the MD needs to be trackable.    *
    */
